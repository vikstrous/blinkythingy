...
func HexStrToColor(str string) *blinkythingy.Color {
	...
	color := blinkythingy.Color{R: rgb[0], G: rgb[1], B: rgb[2]}
	return &color
}

func ParseColorString(name string) *blinkythingy.Color {
...
func MustParseColorString(name string) blinkythingy.Color {
	color := ParseColorString(name)
	if color == nil {
		panic(fmt.Errorf("Failed to parse color: %s", name))
	}
	return *color
}
